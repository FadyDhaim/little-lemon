...
-- index reason: when calling procedure is_table_booked (date, table_number), rather than checking the entire bookings table, we focus on bookings of only the required date, then we check if the required table is occupied on that date
