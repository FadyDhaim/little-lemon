...
select orders.order_id,
       ...
       sum(order_item.total_item_price) as subtotal
from orders
...
from customer
         ...
         natural join menu
where subtotal > 30 -- short cicruit the join process upon joining against the order table, if the subtotal meets the criteria, continue joining the rest of the tables (order_item, menuitem, menu), otherwise skip and move to the next order of whether the same or next customer
order by subtotal;
